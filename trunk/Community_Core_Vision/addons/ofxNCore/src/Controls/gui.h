...
void ofxNCoreVision::setupControls()
{
	...
	trackPanel->addSlider(appPtr->trackedPanel_low_normalizing, "Low level normalizing", 140, 13, 0.0f, 255.0f, 0, kofxGui_Display_Int, 0);
	trackPanel->addSlider(appPtr->trackedPanel_high_normalizing, "High level normalizing", 140, 13, 0.0f, 255.0f, 255, kofxGui_Display_Int, 0);
	trackPanel->addButton(appPtr->trackedPanel_normalizing_on, "Normalizing mode", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_Off, kofxGui_Button_Switch);
	...
	trackPanel->mObjects[5]->mObjX = 11;
	trackPanel->mObjects[5]->mObjY = 100;
	trackPanel->mObjects[6]->mObjX = 185;
	trackPanel->mObjects[6]->mObjY = 100;
		trackPanel->mObjects[7]->mObjX = 167;
	trackPanel->mObjects[7]->mObjY = 11;
	...
	srcPanel->addButton(appPtr->sourcePanel_recording, "Recording mode", OFXGUI_BUTTON_HEIGHT, OFXGUI_BUTTON_HEIGHT, kofxGui_Button_Off, kofxGui_Button_Switch);
	...
	srcPanel->mObjects[2]->mObjX = 110;
	/*srcPanel->mObjects[2]->mObjY = 42;
	srcPanel->mObjects[3]->mObjX = 230;
	...
	****************************/
	...
	controls->update(appPtr->trackedPanel_low_normalizing, kofxGui_Set_Bool, &appPtr->filter->normalizingLowLevel, sizeof(float));
	controls->update(appPtr->trackedPanel_high_normalizing, kofxGui_Set_Bool, &appPtr->filter->normalizingHighLevel, sizeof(float));
	controls->update(appPtr->trackedPanel_normalizing_on, kofxGui_Set_Bool, &appPtr->filter->isNormalizingOn, sizeof(bool));
...
void ofxNCoreVision ::handleGui(int parameterId, int task, void* data, int length)
{
	switch(parameterId)
	{
		case sourcePanel_recording:
		    multiplexer->changeRecordingMode();
			break;
		...
		case calibrationPanel_calibrate:
			if (!bCalibration)
			{
				...
				multiplexer->setCalibrationCamera(0);
