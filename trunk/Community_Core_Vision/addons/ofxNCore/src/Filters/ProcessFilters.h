...
class ProcessFilters : public Filters {
  ...
  public:
    ...
    void allocate( int w, int h ) 
	{
        ...
        normalizedImg.allocate(camWidth, camHeight);
			//GPU Setup
 ...
 ****************************************************************/
     void applyCPUFilters(CPUImageFilter& img){
		...
		normalizedImg = grayBg;//.setFromPixels(grayBg.,camWidth,camHeight)
		...
		if (isNormalizingOn)
		{
			memset((void*)&backHistogram,0,256*sizeof(unsigned int));
			unsigned char* imageData = normalizedImg.getPixels();
			unsigned char* pictureData = img.getPixels();
			int size = camWidth * camHeight;
			for (int i=0;i<size;i++)
				backHistogram[imageData[i]]++;
			float level = 0;
			int count = 1;
			for (int i=normalizingLowLevel;i<=normalizingHighLevel;i++)
			{
				level+=(i*backHistogram[i]);
				count+=backHistogram[i];
			}
			level/=count;
			for (int i=0;i<size;i++)
			{
				if ((imageData[i] >= normalizingLowLevel )&&(imageData[i] <= normalizingHighLevel))
				{
	
					//float scale = imageData[i]/level;
					//pictureData[i]*=level;
					//if (imageData[i]>level)
					//	imageData[i]-=level;
				}
				imageData[i] = (pictureData[i] > imageData[i]) ? pictureData[i] - 2*imageData[i] +level : 0;
			}
			normalizedImg.setFromPixels(imageData,camWidth,camHeight);
			normalizedImg.flagImageChanged();
		}
		if (isNormalizingOn)
		{
			img = normalizedImg;
 ...
 ****************************************************************/
    void draw()
    {
        ...
        grayImg.draw(30, 15, 320, 240); //source

        ampImg.draw(375, 15, 320, 240); //result
	    //  grayDiff.draw(375, 15, 320, 240); //result
        floatBgImg.draw(30, 392, 128, 96);
        subtractBg.draw(165, 392, 128, 96);
        highpassImg.draw(300, 392, 128, 96);
        grayDiff.draw(435, 392, 128, 96);
		...
		//normalizedImg.draw(375, 15, 320, 240); //result

		//grayBg.draw(375, 15, 320, 240); //result

        //floatBgImg.draw(375, 15, 320, 240);
        //subtractBg.draw(165, 392, 128, 96);
        //highpassImg.draw(300, 392, 128, 96);
        //ampImg.draw(435, 392, 128, 96);
    }
