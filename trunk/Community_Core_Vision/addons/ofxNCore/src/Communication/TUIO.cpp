			...
			else // actually send the blobs
			{
				if(bFingers)
				{
					...
					for(blob = fingerBlobs->begin(); blob != fingerBlobs->end(); blob++)
					{
						...
						set.addFloatArg(bHorizontalInverse ? 1-blob->second.centroid.x : blob->second.centroid.x);	// x
						set.addFloatArg(bVerticalInverse ? 1-blob->second.centroid.y : blob->second.centroid.y);	// y
						set.addFloatArg(bHorizontalInverse ? -blob->second.D.x : blob->second.D.x);			// dX
						set.addFloatArg(bVerticalInverse ? -blob->second.D.y : blob->second.D.y);			// dY
		...
		if(bObjects)
		{
			...
			else
			{
				...
				for(blob_obj = objectBlobs->begin(); blob_obj != objectBlobs->end(); blob_obj++)
				{
					...
					set_obj.addFloatArg(bHorizontalInverse ? 1-blob_obj->second.centroid.x : blob_obj->second.centroid.x);	// x
					set_obj.addFloatArg(bVerticalInverse ? 1-blob_obj->second.centroid.y : blob_obj->second.centroid.y);	// y
					set_obj.addFloatArg(bHorizontalInverse ? -blob_obj->second.D.x : blob_obj->second.D.x);			// dX
					set_obj.addFloatArg(bVerticalInverse ? -blob_obj->second.D.y : blob_obj->second.D.y);			// dY
