...
void ofxNCoreVision::loadXMLSettings()
{
	...
	myTUIO.bHorizontalInverse   = XML.getValue("CONFIG:BOOLEAN:HORIZONTALINVERSE",0);
	myTUIO.bVerticalInverse		= XML.getValue("CONFIG:BOOLEAN:VERTICALINVERSE",0);
...
void ofxNCoreVision::saveSettings()
{
	...
	XML.setValue("CONFIG:CAMERA_0:WIDTH", (int)camWidth);
	XML.setValue("CONFIG:CAMERA_0:HEIGHT", (int)camHeight);
	...
	if (bcamera)
	{
		...
		//check if a firefly, ps3 camera, or other is plugged in
			...
			if (multiplexer == NULL) 
			{
				...
				multiplexer->initializeMultiplexer();
				multiplexer->setFilters(filter);
				if (!multiplexer->isMultiplexerInitialized())
				{
					delete multiplexer; 
					multiplexer = NULL;
				}
...
void ofxNCoreVision::_update(ofEventArgs &e)
{
	...
	if(bcamera) //if camera
	{
			...
			if (multiplexer!=NULL)
			{
				if (calib.calibrate.bNextCamera)
				{
					multiplexer->setCalibrationCamera(multiplexer->getCalibrationCamera()+1);
				}
				bNewFrame = true;
			}
		#else
...
void ofxNCoreVision::getPixels()
{
	...
	if (multiplexer!=NULL)
	{
		unsigned char* data = multiplexer->getCPUStitchedFrame(&camWidth,&camHeight);
		processedImg.setFromPixels(data, camWidth, camHeight);
		if(contourFinder.bTrackFiducials){processedImg_fiducial.setFromPixels(data, camWidth, camHeight);}
	}
...
void ofxNCoreVision::grabFrameToGPU(GLuint target)
{
	...
	if (bcamera)
	{
		...
		glBindTexture(GL_TEXTURE_2D, target);
			...
			if(multiplexer!=NULL)
			{
				glTexSubImage2D(GL_TEXTURE_2D, 0, 0, 0, camWidth, camHeight, GL_RGB, GL_UNSIGNED_BYTE, multiplexer->getCPUStitchedFrame(&camWidth,&camHeight));
			}
			else if(vidGrabber!=NULL)
			{
...
//			myTUIO.blobs.clear();
		...
		case 'l':
			/*
			FILE* output = fopen("cam0.txt","w");
			unsigned int w,h;
			unsigned char d;
			unsigned char* data = multiplexer->getCapturedFrameFromCamera(&w,&h,&d,0);
			for (int i=0;i<w*h;i++)
			{
				fprintf(output,"%i ",((int)data[i]));
			}
			fclose(output);
			
			output = fopen("cam1.txt","w");
			data = multiplexer->getCapturedFrameFromCamera(&w,&h,&d,1);
			for (int i=0;i<w*h;i++)
			{
				fprintf(output,"%i ",((int)data[i]));
			}
			...
			output = fopen("cam2.txt","w");
			data = multiplexer->getCapturedFrameFromCamera(&w,&h,&d,2);
			for (int i=0;i<w*h;i++)
			{
				fprintf(output,"%i ",((int)data[i]));
			}
			...
			output = fopen("cam3.txt","w");
			data = multiplexer->getCapturedFrameFromCamera(&w,&h,&d,3);
			for (int i=0;i<w*h;i++)
			{
				fprintf(output,"%i ",((int)data[i]));
			}
		...
		case 'x': //Exit Calibrating
			...
			{	
				...
				bFullscreen = false;
				multiplexer->setCalibratingMode(false);
			}
			break;
